...
export class GameContainer extends Phaser.GameObjects.Container {
    ...
    public create<T extends Phaser.GameObjects.GameObject>(
        config: BaseConfig,
        ...args: any[]
    ): T {
        ...
        if (this.hasTransform(element)) {
            element.setPosition(
                element.x +
                    this.dimensions.x +
                    config.x * this.dimensions.width,
                element.y +
                    this.dimensions.y +
                    config.y * this.dimensions.height
            );
    ...
    protected hasTransform(
        element: any
    ): element is Phaser.GameObjects.Components.Transform {
        return (
            (element as Phaser.GameObjects.Components.Transform).x !=
                undefined &&
            (element as Phaser.GameObjects.Components.Transform).y != undefined
        );
    }
